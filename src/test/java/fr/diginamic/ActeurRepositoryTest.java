...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParFilmParuAnnee() {
		TypedQuery<Acteur> query = em.createQuery(
				"SELECT a FROM Acteur a " + "JOIN a.roles r " + "JOIN r.film f " + "WHERE f.annee = 2015",
				Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParPays() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a " + "JOIN a.roles r "
				+ "JOIN r.film f " + "JOIN f.pays p " + "WHERE p.nom = :nom", Acteur.class);
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a "
				...
				+ "WHERE r.identite = :identite "
				+ "AND f.annee < 2020 AND f.annee > 2010 ", Acteur.class);
